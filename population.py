...
class Statical(Population):
    """
    This implementation of a population algorithm statically assigns the generation of a source 
    in a node of the topology. It is only invoked in the initialization.
        ...
        for id_entity in sim.topology.nodeAttributes:
            ...
            for ctrl in self.src_control:
                ...
                if entity["model"] == ctrl["model"]:
                    ...
                    for number in range(ctrl["number"]):
                        #print("deploy_sink",number,id_entity,msg)
