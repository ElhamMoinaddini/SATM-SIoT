...
import time
#from trust import TrustComput_procedure


def AtoB_procedure(payload):
    # Custom logic here
    print("Executing AtoB_procedure with parameters:", payload)
    result = payload * 2  # Example: multiplying the payload by 2
    print("*******************result",result)
    return result
def BtoC_procedure(payload):
    # Custom logic here
    print("Executing BtoC_procedure with parameters:", payload)
    result = payload * 2  # Example: multiplying the payload by 2
    print("*******************result",result)
    return result
class Message:
    ...
    """
    #print("*******************************Message**")
    def __init__(self, name, src, dst, instructions=0, bytes=0,payload=0, broadcasting=False):
        self.name = name
        self.src = src
        self.dst = dst
        ...
        a={"name": name,
           ...
           "bytes": bytes,
           "payload":payload}
        self.datamsg.append(a)
        #print  ("datamsg====",self.datamsg)
...
def create_applications_from_json(data):
    #print("*******************************create_applications_from_json**")
    applications = {}
    for app in data:
...
class Application:
    ...
    def set_modules(self,data):
        ...
        for module in data:
            ...
            #Device_Type=list(module.values())[0]["Device_Type"]
    ...
    def add_service_source(self, module_name, distribution=None, message=None, module_dest=[], p=[]):
        ...
        #print("*******************************add_service_source**")Message.datamsg
    ...
    def add_service_module(self, module_name, message_in, message_out="", distribution="", module_dest=[], p=[],
        ...
        """
        #print("message_in.name= ",module_name,message_in,message_out)
        #print(len(self.data),self.data[-1])
        
        if not module_name in self.services:
            self.services[module_name] = []
            
        #print("**************************add_service_module**")
        if  message_out=="" :         
            a={"type": Application.TYPE_MODULE,
               "dist": distribution, 
               "param": param,
               "message_in": message_in,
               "message_out": message_out,
               #"message_out.payload":message_out.payload,
        ...
        self.services[module_name].append(a)
        #print("****************************",module_name)
        
        
        
        #print (self.services[module_name])
